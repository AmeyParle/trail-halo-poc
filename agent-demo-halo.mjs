...
const GROQ_MODEL = "llama-3.3-70b-versatile"; // free-tier, supports tool calling
...
// --- Real Halo chain (halo-record from npm — same chain format as the
//     Python package; these records verify with either verifier) ---
...
function verifyAuthority(toolName, b) {
  ...
  if (!AUTHORIZED_SCOPE.includes(toolName))
    return { proven:false, reason:`tool "${toolName}" is OUTSIDE the agent's authorized scope` };
...
async function main() {
  ...
  for (const task of tasks) {
    ...
    for (const tc of toolCalls) {
      ...
      const check = verifyAuthority(name, binding);

      // Record into a REAL Halo chain — principal, decision and the binding
      // itself all land in fields halo-record already defines.
      recorder.append(build("tool_call", "security", {
        tool: name,
        toolInput: args,
        ...
        data: { authority_binding: binding, authority_reason: check.reason },
      }));
      if (check.proven) {
  ...
  // Verify the chain with HALO'S OWN verifier
